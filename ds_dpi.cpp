...
// stop the compiler from optimizing stop because it could
// be changed from outside the program
...
// Debug mode to help with bugfixing
int debug = false;
double ts;
double tot_usec;
...
{
  // Lets trace the direction of the packets
  ...
  if (src_port == dst_port) {
    // If the src adress equal destination adress
    ...
    //hints at the server port in the protocol
...
// Used to demonstate the pointer for the flow
...
typedef struct ident {
  ...
  // stats for incoming and outcoming
  ...
  //record information about the flow, required for the protocol identification process.
	lpi_data_t lpi;
} IdentFlow;
...
void display_ident(Flow *f, IdentFlow *ident)
{
  ...
  // Guess the protocol from the data
...
static void process_packet(libtrace_packet_t *packet)
{
  ...
  // a protocol supported by libprotoident
  ...
  /* A packet is divided into layers
  
    Layer 2/Link layer/Datalink header -- This can be retrieved by trace_get_layer2(),
     or skipped using trace_get_payload_from_layer2().

    Layer 3/IP/IPv6 -- This can be retrieved by trace_get_layer3().
     As a convenience trace_get_ip()/trace_get_ip6() can be used to find an IPv4/IPv6 header.

    Layer 5/transport -- These are protocols carried in IPv4/IPv6 frames.
...
*/
  ...
  // Store the lpi data into the flow sing is_new is now true
  if (is_new) {
    total_flows++;
  ...
  if (protocol->protocol == 215) {
	  if (debug) printf("Expiring STUN!\n");
	    ...
	    fm->updateFlowExpiry(f,packet,dir, -1);
...
int main(int argc, char *argv[])
{
  ...
  if (fm->setConfigOption(LFM_CONFIG_IGNORE_RFC1918, &opt_false) == 0)
		return -1;
